...
test.describe('Login Tests: @authorization', () =>{
    ...
    test('Test Case 1: Login User with correct email and password.', {tag: '@smoke'}, async ({basePage, loginPage}) => {
    ...
    test('Test Case 2: Login User with incorrect password.',  {tag: '@smoke'}, async ({loginPage}) => {
        await loginPage.emailPlaceholder.fill(process.env.LOGIN as string);
    ...
    test('Test Case 3:  Login User with incorrect email.', async ({loginPage}) => {
    ...
    test('Test Case 4:  Login User with invalid email.', async ({loginPage}) => {
